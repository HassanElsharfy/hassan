...
@section('content')
        ...
        <div class="card card-custom">
            <div class="flex-wrap pb-0 border-0 card-header">
                <div class="card-title">
                    <h3 class="card-label">عرض قائمة slider: {{ App\Models\Slider::count() }} </h3>
                </div>
                <div class="card-toolbar">
                    <!--begin::Button-->
                    <a href="{{ route('slider.create', request()->locale) }}" class="btn btn-primary font-weight-bolder">
                ...
                <table id="example" class="table dt-responsive">
                    <thead class="thead_dark">
                        <tr>
                            ...
                            <th>{{ __('maintenance_button') }}</th>
                            <th>{{ __('materials_button') }}</th>
                            <th>{{ __('question') }}</th>
                            <th>الخلفية</th>
                            <th>اللوجو</th>
                            <th>الإجراءات</th>
                    ...
                    <tbody>
                        @foreach ($sliders as $slider)
                            <tr>
                                ...
                                <td>{{ $slider->maintenance_button }}</td>
                                <td>{{ $slider->materials_button }}</td>
                                <td>{{ $slider->question }}</td>

                                <td> <img src="{{ asset('filebackgrounds') }}/{{ $slider->background }}"
                                        alt="{{ $slider->main_title }}" width="150px" height="50px"></td>

                                <td><img src="{{ asset('filelogos') }}/{{ $slider->logo }}"
                                        alt="{{ $slider->main_title }}" width="150px" height="50px"></td>

                                <!-- Actions your table rows here... -->
                                <td data-field="Actions" data-autohide-disabled="false" aria-label="null"
                                    ...
                                    <span style="overflow: visible; position: relative; width: 110px;">
                                        ...
                                        {{-- <a href="{{ url(request()->locale.'/slider/' . $sliders . '/edit') }}"
                                            class="btn btn-sm btn-clean btn-icon" title="Edit details"><i
                                                class="la la-edit"></i></a> --}}
                                                {{-- <a href="{{ route('slider.edit', ['locale' => request()->locale, 'slider' => $slider->id]) }}"
   ...
   <i class="la la-edit"></i>
                                            ...
                                            class="btn btn-sm btn-clean btn-icon" title="Edit details"><i
                                                class="la la-edit"></i></a> --}}
{{--
                                                <a href="{{ route('slider.edit',$slider) }}" type="button"
                                                    class="btn btn-info"><i class="fas fa-edit"></i></a> --}}

                                        <a
                                        {{-- href="{{ url('slider/' . $slider->id) }}" --}}
                                        href="{{ route('slider.show', ['locale' => request()->locale, 'slider' => $slider->id]) }}"
                                            title="View slider"><button class="btn btn-sm btn-clean btn-icon"> <svg
                                                    xmlns="http://www.w3.org/2000/svg" width="16" height="16"
